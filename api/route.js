import { laneModel } from '../lib/openrouter-lanes.js';
...
export function classify(mission='') {
  ...
  if (JUDGE.some(x => text.includes(x))) return {taskType:'judge', lane:'judge', model:laneModel('judge'), maxTokens:1400};
  ...
  if (risk || (reasoning && words.length >= 28)) return {taskType:'reasoning', lane:'reasoner', model:laneModel('reasoner'), maxTokens:1100};
  if (worker) return {taskType:'worker', lane:'worker', model:laneModel('worker'), maxTokens:700};
  if (reasoning) return {taskType:'reasoning_light', lane:'worker', model:laneModel('worker'), maxTokens:700};
  return {taskType:'default', lane:'flash', model:laneModel('flash'), maxTokens:320};
}
...
export default function handler(req,res){
  ...
  res.status(200).json({ok:true,router:'BARS Router V2',routing:'deterministic',gateway:'OpenRouter (BARS lane routing)',...classify(mission)});
}
