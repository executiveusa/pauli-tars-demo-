...
set -e
rm -rf /tmp/v5-data /tmp/v5-rec /tmp/v5-deploy /tmp/barsdata2 /tmp/adv-data /tmp/v3-data /tmp/v4-* /tmp/adv-deploy 2>/dev/null || true
cd "$(dirname "$0")/.."
for t in test_bars test_security test_paid test_adversarial test_fourth test_fifth test_sixth; do
  echo "===== tests/$t.py ====="
