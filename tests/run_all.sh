...
cd "$(dirname "$0")/.."
for t in test_bars test_security test_paid test_adversarial test_fourth test_fifth test_sixth test_seventh test_eighth test_ninth test_tenth test_memory_v2 test_verify_loop test_tool_contracts test_skills_wiring test_mission_packaging test_composio_runtime_contract test_jev_hands_contract test_terabithia_adapter; do
  echo "===== tests/$t.py ====="
  pkill -f mock_provider.py 2>/dev/null || true; pkill -f "python3 server.py" 2>/dev/null || true; sleep 0.5
  python3 "tests/$t.py"
done
node tests/test_bars_capabilities.mjs
python3 scripts/check_browser_hands_external.py
