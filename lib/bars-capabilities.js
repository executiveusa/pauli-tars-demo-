...
export const BARS_CAPABILITIES = [
  ...
  },
  {
    ...
    label: 'HyperFrames Video Engine',
    tier: 'tool',
    transport: 'loopback-sidecar',
    source: 'heygen-com/hyperframes',
    purpose: 'Lint and render agent-authored HTML compositions to MP4/WebM/MOV/GIF on the VPS (media/hyperframes). Returns the file with its sha256; a failed render is reported, never claimed.',
  },
  {
    id: 'jev-hands',
...
export function publicCapabilityStatus(env = process.env) {
  ...
  const hyperframesConfigured = Boolean(env.HYPERFRAMES_RENDER_URL && (env.HYPERFRAMES_RENDER_TOKEN || '').length >= 32);
  ...
  return BARS_CAPABILITIES.map((capability) => ({
    ...
    status:
      capability.id === 'hermes-core' || capability.id === 'pauli-skill-pack'
        ...
        : capability.id === 'hyperframes-video'
          ? (hyperframesConfigured ? 'configured' : 'declared')
