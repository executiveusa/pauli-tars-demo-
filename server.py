...
REPOS_DIR = os.path.join(DATA, "repos")   # connected repos (CONNECT TRAIL MIXX)
...
def provider_models():
    ...
    try:
        ...
        if "openrouter" in base:
            ...
            ids = free or ids   # his lane: free models only; paid ids would just 402
...
RIME_SPEAKERS = {"bond", "orion", "luna", "celeste", "astra"}  # live-verified on arcana 2026-09-20

def _rime_tts(text, speaker=None):
    ...
    sits inside the existing /tts confirmation + budget controls. Optional
    per-call speaker, whitelist-gated to live-verified Rime voices."""
    key = os.environ.get("RIME_API_KEY") or os.environ.get("RIME_API_TOKEN")
    if not key:
        return None
    spk = (speaker if speaker in RIME_SPEAKERS else None) \
        or os.environ.get("RIME_VOICE") or os.environ.get("RIME_SPEAKER") or "bond"
...
def tts_bytes(text, speaker=None):
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _demo_tts(self):
        ...
        spk = str(data.get("speaker") or "").strip().lower()
        if not _demo_voice_ok(sid):
            ...
            return
        try:
            audio, ctype = tts_bytes(text, spk or None)
        except Exception as e:
            _receipt({"kind": "demo", "lane": "demo.tts", "ok": False,
                      "error": str(e)[:160]})
    ...
    def do_GET(self):
        ...
        if path in ("/", "/frontdoor", "/frontdoor.html", "/frontdoor/", "/agent", "/agent/", "/index.html", "/ops"):
            ...
            try:
                ...
                pass
            self._json({"ok": True, "repos": repos})
            return
        elif path == "/api/fleet":
    ...
    def do_POST(self):
        ...
        if path in ("/chat", "/api/chat"):
            ...
            dest = os.path.join(REPOS_DIR, slug)
            tok = os.environ.get("BARS_REPO_TOKEN", "")
            try:
                os.makedirs(REPOS_DIR, exist_ok=True)
                hdrs = {"Authorization": "token " + tok} if tok else {}
                req = urllib.request.Request(
                    f"https://codeload.github.com/executiveusa/{slug}/tar.gz/HEAD",
                    headers=hdrs)
                ...
                _receipt({"kind": "repo", "lane": "repos.connect", "ok": True,
                          "repo": slug, "files": n})
                self._json({"ok": True, "repo": slug, "files": n})
            except Exception as e:
                _receipt({"kind": "repo", "lane": "repos.connect", "ok": False,
                          "repo": slug, "error": str(e)[:160]})
                self._json({"ok": False, "repo": slug, "error": str(e)[:200]})
            return
        elif path == "/tts":
